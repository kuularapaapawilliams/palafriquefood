...
def vendor_detail(request, vendor_slug):
    ...
    #display current day opening hour
...
def add_to_cart(request, food_id):
    if request.user.is_authenticated:
        ...
        #CHECK IF FOOD ITEM EXIST
            try:
                ...
                try:
                    ...
                    return JsonResponse({'status': 'Success', 'message': 'FoodItem added to cart', 'cart_counter': get_cart_counter(request), 'qty': chkCart.quantity, 'cart_amount': get_cart_amounts(request)})
        ...
        return JsonResponse({'status': 'login_required', 'message': 'Please login to continue adding to cart'})
...
def search(request):
    if not 'address' in request.GET:
        ...
        address = request.GET['address'] #uncoment this after working on google API
        latitude = request.GET['lat']
        longitude = request.GET['lng']
        radius = request.GET['radius']
        keyword = request.GET['keyword'] 
        # get vendors id that has the food item the user is looking for
        ...
        return render(request, 'marketplace/listings.html', context) #Dr. Hilla Limann Hall
...
def checkout(request):
    ...
    default_values = {
        ...
        'pin_code': user_profile.postal_code,
